...
check("WebAssembly.Memory/Global/Table", () => { const m = new WebAssembly.Memory({ initial: 1 }); if (!(m.buffer instanceof ArrayBuffer) || m.buffer.byteLength !== 65536) throw new Error("Memory"); const G = (WebAssembly as any).Global; if (typeof G === "function") { const g = new G({ value: "i32", mutable: true }, 42); if (g.value !== 42) throw new Error("Global"); } });
