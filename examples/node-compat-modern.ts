...
check("crypto sha512-256 + RSA padding constants + sign-with-options", () => { const c = require("node:crypto"); if (c.createHash("sha512-256").update("abc").digest("hex").length !== 64) throw new Error("sha512-256"); if (c.constants.RSA_PKCS1_PSS_PADDING === undefined || c.constants.RSA_PKCS1_OAEP_PADDING === undefined) throw new Error("constants"); const { privateKey, publicKey } = c.generateKeyPairSync("rsa", { modulusLength: 2048 }); const sig = c.sign("sha256", "data", { key: privateKey, padding: c.constants.RSA_PKCS1_PSS_PADDING, saltLength: 32 }); if (!c.verify("sha256", "data", { key: publicKey, padding: c.constants.RSA_PKCS1_PSS_PADDING, saltLength: 32 }, sig)) throw new Error("pss verify"); });
...
check("crypto HMAC/cipher key from KeyObject (jsonwebtoken security)", () => { const c = require("node:crypto"); const ko1 = c.createSecretKey(Buffer.from("secret")), ko2 = c.createSecretKey(Buffer.from("wrong")); const h1 = c.createHmac("sha256", ko1).update("d").digest("hex"); const h2 = c.createHmac("sha256", ko2).update("d").digest("hex"); const hs = c.createHmac("sha256", "secret").update("d").digest("hex"); if (h1 === h2) throw new Error("KeyObject HMAC ignores key (would accept any JWT secret!)"); if (h1 !== hs) throw new Error("KeyObject != string key"); });
check("process.setSourceMapsEnabled", () => { if (typeof process.setSourceMapsEnabled !== "function") throw new Error("missing"); process.setSourceMapsEnabled(true); process.setSourceMapsEnabled(false); });
...
check("node:inspector module surface", () => { const i = require("node:inspector"); if (typeof i.Session !== "function" || typeof i.open !== "function" || typeof i.url !== "function") throw new Error("inspector"); const s = new i.Session(); s.connect(); s.disconnect(); });
check("AsyncLocalStorage run/getStore", () => { const { AsyncLocalStorage } = require("node:async_hooks"); const als = new AsyncLocalStorage(); if (als.run({ id: 7 }, () => als.getStore().id) !== 7) throw new Error("run"); if (als.getStore() !== undefined) throw new Error("leaked"); });
check("util.getSystemErrorName", () => { const u = require("node:util"); if (u.getSystemErrorName(-2) !== "ENOENT" || u.getSystemErrorName(-13) !== "EACCES") throw new Error("name"); if (!(u.getSystemErrorMap() instanceof Map)) throw new Error("map"); });
check("util.getCallSites", () => { const { getCallSites } = require("node:util"); function lvl() { return getCallSites(); } const sites = lvl(); if (!Array.isArray(sites) || sites.length < 2) throw new Error("array"); if (sites[0].functionName !== "lvl") throw new Error("fn:" + sites[0].functionName); if (typeof sites[0].lineNumber !== "number" || typeof sites[0].scriptName !== "string") throw new Error("shape"); });
check("process.setUncaughtExceptionCaptureCallback", () => { if (process.hasUncaughtExceptionCaptureCallback() !== false) throw new Error("initial"); const fn = () => {}; process.setUncaughtExceptionCaptureCallback(fn); if (process.hasUncaughtExceptionCaptureCallback() !== true) throw new Error("set"); process.setUncaughtExceptionCaptureCallback(null); });
check("crypto.subtle RSA-PSS sign/verify", async () => { const kp: any = await crypto.subtle.generateKey({ name: "RSA-PSS", modulusLength: 2048, publicExponent: new Uint8Array([1, 0, 1]), hash: "SHA-256" }, true, ["sign", "verify"]); const data = new TextEncoder().encode("d"); const sig = await crypto.subtle.sign({ name: "RSA-PSS", saltLength: 32 }, kp.privateKey, data); if (!(await crypto.subtle.verify({ name: "RSA-PSS", saltLength: 32 }, kp.publicKey, sig, data))) throw new Error("verify"); });
check("process.config + versions", () => { if (typeof process.config !== "object" || !process.config.variables) throw new Error("config"); if (!process.versions.node || typeof process.versions.v8 !== "string") throw new Error("versions"); });
check("zlib.crc32", () => { const z = require("node:zlib"); if (z.crc32("hello") !== 907060870) throw new Error("crc=" + z.crc32("hello")); if (z.crc32("") !== 0) throw new Error("empty"); });
check("assert.throws/rejects with matcher object", async () => { const a = require("node:assert"); a.throws(() => { throw new TypeError("bad value"); }, { name: "TypeError", message: /bad/ }); await a.rejects(async () => { throw new Error("nope"); }, /nope/); let ok = false; try { a.throws(() => { throw new Error("x"); }, { name: "TypeError" }); } catch { ok = true; } if (!ok) throw new Error("should have rejected wrong name"); });
check("node:punycode", () => { const p = require("node:punycode"); if (p.toASCII("münchen.de") !== "xn--mnchen-3ya.de") throw new Error("toASCII"); if (p.encode("bücher") !== "bcher-kva") throw new Error("encode"); if (p.toUnicode("xn--mnchen-3ya.de") !== "münchen.de") throw new Error("toUnicode"); });
check("util.parseArgs positionals", () => { const { parseArgs } = require("node:util"); const { values, positionals } = parseArgs({ args: ["--foo", "1", "p1", "p2"], options: { foo: { type: "string" } }, allowPositionals: true }); if (values.foo !== "1" || positionals.length !== 2) throw new Error(JSON.stringify({ values, positionals })); });
...
check("util.toUSVString + transferableAbort*", () => { const util = require("node:util"); if (util.toUSVString("ab\uD800cd") !== "ab�cd") throw new Error("toUSVString=" + JSON.stringify(util.toUSVString("ab\uD800cd"))); if (util.toUSVString("ok") !== "ok") throw new Error("usv plain"); const ac = util.transferableAbortController(); if (!ac.signal || ac.signal.aborted) throw new Error("tac"); if (util.transferableAbortSignal(ac.signal) !== ac.signal) throw new Error("tas"); });
check("buffer.transcode", () => { const { transcode, Buffer } = require("node:buffer"); const out = transcode(Buffer.from("héllo", "utf8"), "utf8", "latin1"); if (out.toString("latin1") !== "héllo") throw new Error("transcode=" + out.toString("latin1")); });
check("crypto.Hash is a Transform stream (setEncoding/pipe)", async () => { const crypto = require("node:crypto"); const { Readable } = require("node:stream"); const h = crypto.createHash("sha256"); h.setEncoding("hex"); let out = ""; h.on("data", (d: string) => (out += d)); await new Promise<void>((res, rej) => { h.on("end", res); h.on("error", rej); Readable.from(["hello ", "world"]).pipe(h); }); const direct = crypto.createHash("sha256").update("hello world").digest("hex"); if (out !== direct) throw new Error("stream digest " + out + " !== " + direct); });
