...
check("fs.openSync 'wx' exclusive", () => { const fs = require("node:fs"); const os = require("node:os"); const path = require("node:path"); const f = path.join(os.tmpdir(), "velox-wx-" + process.pid); fs.closeSync(fs.openSync(f, "wx")); let threw = false; try { fs.openSync(f, "wx"); } catch (e: any) { threw = e.code === "EEXIST"; } fs.rmSync(f, { force: true }); if (!threw) throw new Error("wx should EEXIST"); });
